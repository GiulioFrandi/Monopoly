 ...
 * proprietà, stazioni e società disponibili. Ogni casella è modellata come un
 * oggetto {@link Proprietà}, con nome, valore e tabella degli affitti.
 * </p>
 *
 * <h2>Ruolo nell'architettura</h2>
 * <ul>
 * <li>Gestisce la disposizione delle caselle del Monopoly.</li>
 * <li>Permette di calcolare la nuova posizione di un {@link Giocatore} in base
 * al lancio dei dadi.</li>
 * <li>Consente di rimuovere una pedina dal tabellone quando un giocatore
 * abbandona la partita.</li>
 * <li>Espone la lista delle caselle per reporting e visualizzazione nelle
 * interfacce utente.</li>
 * </ul>
 *
 * <h2>Note</h2>
 * <ul>
 * <li>La disposizione delle proprietà segue la versione italiana del Monopoly
 * classico.</li>
 * <li>Le stazioni e le società sono modellate anch'esse come
 * {@link Proprietà}.</li>
 * <li>Il tabellone è inizializzato automaticamente nel costruttore tramite
 * {@link #inizializza()}.</li>
 * </ul>
 *
 * @author Giulio
...
public class Tabellone {
     ...
     * <p>
     * Aggiunge tutte le proprietà, stazioni e società con i rispettivi valori e
     * tabelle degli affitti.
     * </p>
     */
     ...
     * <p>
     * Aggiorna la posizione del giocatore e restituisce la casella
     * corrispondente.
     * </p>
     *
     ...
     * <p>
     * Imposta la posizione del giocatore a -1 per indicare che non è più
     * attivo.
     * </p>
     *
     ...
     */
    public List<Proprietà> getCaselle() {
        return caselle;
    }

    /**
     * Restituisce una rappresentazione testuale del tabellone mostrando per
     * ogni casella quali giocatori si trovano lì.
     *
     * @param giocatori lista dei giocatori attivi
     * @return stringa formattata del tabellone
     */
    public String getTabelloneConGiocatori(List<Giocatore> giocatori) {
        StringBuilder sb = new StringBuilder();

        for (int i = 0; i < caselle.size(); i++) {
            Proprietà p = caselle.get(i);

            sb.append(i).append(") ").append(p.getNome());

            // Trova i giocatori su questa casella
            List<String> presenti = new ArrayList<>();
            for (Giocatore g : giocatori) {
                if (g.getPosizione() == i) {
                    presenti.add(g.getUtente().getUsername());
                }
            }

            if (!presenti.isEmpty()) {
                sb.append("  <-- Giocatori: ").append(String.join(", ", presenti));
            }
